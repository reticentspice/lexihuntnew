...
//3. Set up other pregame stuff: millions of variables plus window.onload
let goalNumber = Math.floor(Math.random() * 16) + 30;
let letterValues = {};
let letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
for (let i = 0; i < letters.length; i++) {
    let letter = letters[i];
    letterValues[letter] = Math.floor(Math.random() * 8) + 1;
}
...
function printGuess(userGuess, guessTotal, guessNumber) {
    ...
    //Generate hints
    if (guessNumber <= 3) {
        var randomLetter = hardList[Math.floor(Math.random() * hardList.length)];
        var randomLetterValue = letterValues[randomLetter];
        const index = hardList.indexOf(randomLetter);
        hardList.splice(index, 1);
        const newHint = document.createElement('p');
        newHint.setAttribute("Id", "hintField");
        newHint.textContent = `Hint: ${randomLetter} = ${randomLetterValue}`;
        guessDiv.appendChild(newHint);
    }
    else if (guessNumber <= 6) {
        var randomLetter = midList[Math.floor(Math.random() * midList.length)];
        var randomLetterValue = letterValues[randomLetter];
        ...
        var randomLetter = easyList[Math.floor(Math.random() * easyList.length)];
        var randomLetterValue = letterValues[randomLetter];
